...
MemWriter::MemWriter(SharedMemory* pSharedMemory) :
...
HRESULT MemWriter::GetFileName(LPWSTR *lpszFileName)
{
	*lpszFileName = m_pFileName;
	return S_OK;
}

HRESULT MemWriter::SetFileName(LPCWSTR pszFileName)
{
...
BOOL MemWriter::IsFileInvalid()
{
	return (m_hFile == INVALID_HANDLE_VALUE);
}

DWORD MemWriter::SetFilePointer(__int64 llDistanceToMove, DWORD dwMoveMethod)
{
	LARGE_INTEGER li;
...
__int64 MemWriter::GetFilePointer()
{
	LARGE_INTEGER li;
	li.QuadPart = 0;
...
HRESULT MemWriter::Write(PBYTE pbData, ULONG lDataLength)
{
	HRESULT hr;
	...
	DWORD written = 0;
	hr = m_pSharedMemory->WriteFile(m_hFile, (PVOID)pbData, (DWORD)lDataLength, &written, NULL);
...
void MemWriter::SetChunkReserve(BOOL bEnable, __int64 chunkReserveSize, __int64 maxFileSize)
{
