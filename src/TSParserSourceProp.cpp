...
CUnknown * WINAPI CTSParserSourceProp::CreateInstance(LPUNKNOWN pUnk, HRESULT *pHr)
{
	CTSParserSourceProp *pNewObject = new CTSParserSourceProp(pUnk);
	if (pNewObject == NULL)
	{
		*pHr = E_OUTOFMEMORY;
	}
...
CTSParserSourceProp::CTSParserSourceProp(IUnknown *pUnk) :
	CBasePropertyPage(NAME("TSParserSourceProp"), pUnk, IDD_PARSERPROPPAGE, IDS_PARSERPROPPAGE_TITLE),
	m_pProgram(0)
{
	m_dRate = 1.0;
}

CTSParserSourceProp::~CTSParserSourceProp(void)
{
...
DWORD CTSParserSourceProp::ThreadProc(void)
{
	BrakeThread Brake;
...
// DoProcessingLoop
//
HRESULT CTSParserSourceProp::DoProcessingLoop(void)
{
    Command com;
...
BOOL CTSParserSourceProp::ThreadRunning(void)
{ 
	return m_bThreadRunning;
}

HRESULT CTSParserSourceProp::OnConnect(IUnknown *pUnk)
{
	if (pUnk == NULL)
	{
		return E_POINTER;
	...
	HRESULT hr = pUnk->QueryInterface(IID_ITSParserSource, (void**)(&m_pProgram));
...
HRESULT CTSParserSourceProp::OnDisconnect(void)
{
	if (CAMThread::ThreadExists())
	{
		CAMThread::CallWorker(CMD_STOP);
...
HRESULT CTSParserSourceProp::OnActivate(void)
{
	ASSERT(m_pProgram != NULL);
	PopulateDialog();
	HRESULT hr = S_OK;
...
void CTSParserSourceProp::SetDirty()
{
	m_bDirty = TRUE;
...
void CTSParserSourceProp::SetClean()
{
	m_bDirty = FALSE;
...
BOOL CTSParserSourceProp::OnRefreshProgram()
{
	PopulateDialog();
	return TRUE;
}

BOOL CTSParserSourceProp::PopulateDialog()
{
	ASSERT(m_pProgram != NULL);
...
BOOL CTSParserSourceProp::RefreshDialog()
{
	ASSERT(m_pProgram != NULL);
...
HRESULT CTSParserSourceProp::OnApplyChanges(void)
{

	TCHAR sz[100];
	sprintf(sz, "%S", L"Do you wish to save these Filter Settings");
	if (MessageBox(NULL, sz, TEXT("TSParserSource Filter Settings"), MB_YESNO) == IDYES)
	{
		m_pProgram->SetRegSettings();
		m_pProgram->SetRegProgram();
		OnRefreshProgram ();
