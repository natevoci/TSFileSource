...
// CTSParserSinkPin
//////////////////////////////////////////////////////////////////////
CTSParserSinkPin::CTSParserSinkPin(CTSParserSink *pTSParserSink, LPUNKNOWN pUnk, CBaseFilter *pFilter, CCritSec *pLock, HRESULT *phr) :
...
CTSParserSinkPin::~CTSParserSinkPin()
{
	Clear();
	delete[] m_writeBuffer;
}

void CTSParserSinkPin::Clear()
{
	StopThread(0);
...
HRESULT CTSParserSinkPin::BreakConnect()
{
	Clear();

    if (m_pTSParserSink->m_pPosition != NULL) {
        m_pTSParserSink->m_pPosition->ForceRefresh();
   }
//Frodo code changes
    m_restBufferLen=0;
//Frodo code changes
...
STDMETHODIMP CTSParserSinkPin::ReceiveCanBlock()
{
    return S_FALSE;
}

HRESULT CTSParserSinkPin::CheckMediaType(const CMediaType *)
{
    return S_OK;
}

//Frodo code changes
HRESULT CTSParserSinkPin::Filter(byte* pbData,long sampleLen)
{
	HRESULT hr;
	int packet = 0;
	BOOL bProg = FALSE;
...
void CTSParserSinkPin::ThreadProc()
{
	...
	while (!ThreadIsStopping(0))
	{
		...
		if (item)
		{
			HRESULT hr = m_pTSParserSink->Write(item, sampleLen);
			delete[] item;
			if (FAILED(hr))
			{
				CAutoLock BufferLock(&m_BufferLock);
				::OutputDebugString(TEXT("TSParserSinkPin::ThreadProc:Write Fail."));
				std::vector<CBufferInfo*>::iterator it = m_Array.begin();
				for ( ; it != m_Array.end() ; it++ )
				{
					CBufferInfo *bufferInfo = *it;
...
HRESULT CTSParserSinkPin::WriteBufferSample(byte* pbData,long sampleLen)
{
	HRESULT hr;
	...
	if (!m_WriteThreadActive)
	{
		...
		hr = m_pTSParserSink->Write(pbData, sampleLen);
		return hr;
	}
	...
	if(m_WriteThreadActive && m_WriteBufferSize + sampleLen < 64000000)
	{
		...
		if(sampleLen <= bufferLen)
		{
			...
			if (!newItem->sample)
			{
				::OutputDebugString(TEXT("TSParserSinkPin::WriteBufferSample:Out of Memory."));
				return S_OK;
			}
	...
	::OutputDebugString(TEXT("TSParserSinkPin::WriteBufferSample:Buffer Full error."));
	CAutoLock BufferLock(&m_BufferLock);
	::OutputDebugString(TEXT("TSParserSinkPin::ThreadProc:Write Fail."));
	std::vector<CBufferInfo*>::iterator it = m_Array.begin();
	for ( ; it != m_Array.end() ; it++ )
...
STDMETHODIMP CTSParserSinkPin::Receive(IMediaSample *pSample)
{
...
STDMETHODIMP CTSParserSinkPin::EndOfStream(void)
{
	Clear();
    CAutoLock lock(&m_ReceiveLock);
...
STDMETHODIMP CTSParserSinkPin::NewSegment(REFERENCE_TIME tStart, REFERENCE_TIME tStop, double dRate)
{
	Clear();
	m_PacketErrors = 0;
	m_restBufferLen=0;
...
__int64 CTSParserSinkPin::getNumbErrorPackets()
{
    return m_PacketErrors;
}

void CTSParserSinkPin::setNumbErrorPackets(__int64 lpllErrors)
{
    m_PacketErrors = lpllErrors;
}

void CTSParserSinkPin::PrintLongLong(LPCTSTR lstring, __int64 value)
{
	TCHAR sz[100];
